...
Every HTTP request MUST receive a UUID v4 correlation ID attached to `request.state.correlation_id`. On completion, the middleware MUST log a `request_completed` event.
